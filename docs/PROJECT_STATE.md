...
> - ~~**3 stale `sync_review` rows**~~ ✅ **RESOLVED 9/14** with a note naming what superseded them.
>   The queue now holds exactly what it should: **2 `zoom-attendance-noshow`** (the worklist Zach
>   asked for — Mohamed Hagras and Shane Ison, a notetaker bot alone in the room) and **6
>   `zoom-no-occurrence`**.
> - ~~**The "Milestone Grant Meeting" calendar is unrouted**~~ ✅ **DECIDED 9/14 — leave it unrouted.**
>   Zach: *"I want to ignore the milestone grant meeting calendar."* Its 4 Zoom meetings in 180 days
>   stay outside both the note writer and activity ingestion. Not an oversight; do not "fix" it.
...
> `zoomNotes.ts:146`) while printing "DRY RUN — no writes made" — ✅ **the message was corrected 9/14**
> to say "no GHL writes" and to name the rows it did record. The behaviour was kept deliberately:
> deduped, and surfacing anomalies during a review is the point.
>
